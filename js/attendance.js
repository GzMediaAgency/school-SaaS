...
render() {
     ...
     this.contentArea.innerHTML = `
         <div class="card" style="padding: 1.5rem; margin-bottom: 1.5rem;">
             <form id="attendanceFilterForm" style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
                 ...
                 <div style="align-self: flex-end; display: flex; gap: 0.5rem;">
                     <button type="submit" class="btn btn-primary" id="loadAttendanceBtn">تحميل القائمة</button>
                     <button type="button" class="btn btn-outline" id="monthlyReportBtn">📊 التقرير الشهري</button>
                 </div>
             </form>
         </div>
         ...
         <div class="modal" id="monthlyReportModal">
             <div class="modal-content" style="max-width: 900px;">
                 ...
                 <div style="margin-bottom: 1rem; display: flex; gap: 1rem; align-items: center;">
                     ...
                     <button class="btn btn-success" id="exportReportBtn">🖨️ تصدير PDF</button>
                 </div>
                 <div id="monthlyReportContent" style="overflow-x: auto;">
                 </div>
             </div>
         </div>
     `;
 ...
 loadAttendance() {
     ...
     const existingAttendance = db.getData('attendance').filter(a => 
 ...
 generateMonthlyReport(month, classId) {
     const students = db.getData('students').filter(s => s.classId === classId);
     const allAttendance = db.getData('attendance');
     ...
     students.forEach(s => {
         let absentCount = 0;
         let totalDays = 0;
         html += `<tr><td style="border: 1px solid #ddd; padding: 0.5rem; font-weight: bold;">${s.fullName}</td>`;
         for (let d = 1; d <= daysInMonth; d++) {
             const dateStr = `${year}-${String(monthNum).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
             const record = allAttendance.find(a => a.studentId === s.id && a.date === dateStr);
             if (record) {
                 totalDays++;
 ...
 exportMonthlyReport(month, classId) {
     const students = db.getData('students').filter(s => s.classId === classId);
     const allAttendance = db.getData('attendance');
     const classes = db.getData('classes');
     ...
     headers.push('إجمالي الغياب', 'نسبة الغياب');

     const rows = students.map(s => {
         const row = [s.fullName];
         let absentCount = 0;
         let totalDays = 0;
         for (let d = 1; d <= daysInMonth; d++) {
             const dateStr = `${year}-${String(monthNum).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
             ...
             if (record) {
                 totalDays++;
                 if (record.status === 'absent') { absentCount++; row.push('غائب'); }
         ...
         const rate = totalDays > 0 ? ((absentCount / totalDays) * 100).toFixed(0) : 0;
         row.push(String(absentCount), `${rate}%`);
         return row;
     });

     const title = `تقرير الغياب الشهري - ${cls ? cls.name : ''} - ${monthNames[monthNum - 1]} ${year}`;
 ...
 attachEvents() {
     ...
     document.getElementById('generateReportBtn').addEventListener('click', () => {
         ...
         document.getElementById('monthlyReportContent').innerHTML = this.generateMonthlyReport(month, this.selectedClass);
     });
     document.getElementById('exportReportBtn').addEventListener('click', () => {
         ...
         this.exportMonthlyReport(month, this.selectedClass);
     });
 }
 saveAttendance() {
     const students = db.getData('students').filter(s => s.classId === this.selectedClass);
     ...
     const filtered = allAttendance.filter(a => {
         ...
         return !(a.date === this.selectedDate && student);
     });
     ...
     db.setData('attendance', filtered);
