...
class AIHelper {
    ...
    generateInsights() {
        ...
        const grades = db.getData('grades');
        ...
        students.forEach(s => {
            ...
            if (studentGrades.length > 0) {
                ...
                if (avg < 10) failingStudents.push(s.fullName);
            }
        });
        ...
        if (balance < 0) {
            insights.push({
                icon: '💸',
                title: 'عجز مالي',
                message: `المصاريف تتجاوز المداخيل بمقدار ${ui.formatNumber(Math.abs(balance))} دج. يجب مراجعة الميزانية.`,
                color: 'var(--danger-red)',
                action: 'finance'
            });
        } else {
            insights.push({
                ...
                message: `الرصيد الحالي إيجابي بمقدار ${ui.formatNumber(balance)} دج. المؤسسة تتمتع باستقرار مالي.`,
                color: 'var(--success-green)',
                action: 'finance'
            });
        }
