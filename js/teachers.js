...
render() {
 ...
 saveTeacher() {
     ...
     if (id) {
         ...
         this.syncFinanceFromTeacher(id, data.fullName, data.salary);
         ui.showToast('تم تحديث بيانات الأستاذ بنجاح', 'success');
     } else {
         const created = db.addItem('teachers', data);
         this.syncFinanceFromTeacher(created.id, data.fullName, data.salary);
         ui.showToast('تم إضافة الأستاذ بنجاح', 'success');
     }
     ui.closeModal('teacherModal');
     this.loadTeachers();
 }
 // يزامن راتب الأستاذ كمصروف تلقائي في المالية عند الإضافة أو التعديل
 syncFinanceFromTeacher(teacherId, teacherName, salary) {
     const finance = db.getData('finance');
     const key = `salary:${teacherId}`;
     const existing = finance.find(f => f.linkedTeacherKey === key);
     ...
     if (salary > 0) {
         const financeData = {
             ...
             amount: salary,
             description: `راتب الأستاذ ${teacherName}`,
             date: new Date().toISOString().split('T')[0],
             linkedTeacherKey: key,
             autoGenerated: true
         };
         if (existing) {
 ...
 async deleteTeacher(id) {
     ...
     if (confirmed) {
         ...
         const linked = db.getData('finance').find(f => f.linkedTeacherKey === `salary:${id}`);
